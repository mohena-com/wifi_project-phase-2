...
class WifiCSIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if hasattr(self, "scaler_meta") and self.scaler_meta is not None:
            m_seq = self.scaler_meta.transform(m_seq)       # (W, 11)
        
        if hasattr(self, "scaler_csi") and self.scaler_csi is not None:
            csi_seq = self.scaler_csi.transform(csi_seq)     # (W, 180 or 99)
        # --------------------------------------------------
        
        subject_label = int(y["subject"][0]) - 1   # 1..30 → 0..29
    ...
    def set_scalers(self, scaler_meta, scaler_csi):
        self.scaler_meta = scaler_meta
        self.scaler_csi = scaler_csi
        print(">>> Scalers attached successfully!")
