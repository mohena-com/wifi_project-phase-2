...
def get_test_train_loaders(train_dataset, test_dataset, batch_size, device):
    ...
    t_shuffle = cr.get_bool("t_shuffle", False)
    v_shuffle = cr.get_bool("v_shuffle", False)
    ...
    print(f"csi_seq: {batch['csi_seq'].shape}, metadata_seq:{batch['metadata_seq'].shape}, label: {batch['label'][:10]}")
